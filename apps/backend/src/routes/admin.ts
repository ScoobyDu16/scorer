...
import { authenticateAdmin, requireSuperAdmin, generateAdminTokens } from "../middleware/admin-auth";
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    const adminUser = await drizzleDb
      .select({
        user: users,
        role: roles,
        userRole: userRoles
    ...
    res.json({
      ...
      data: {
        user: {
          ...
          name: user.name,
          role: role.name,
          turfId: adminUser[0].userRole?.turfId
        },
        tokens: {
          accessToken,
          refreshToken
...
router.get("/dashboard/stats", authenticateAdmin, requireSuperAdmin, async (req, res) => {
  try {
...
router.get("/turfs/pending", authenticateAdmin, requireSuperAdmin, async (req, res) => {
  try {
...
router.post("/turfs/:turfId/verify", authenticateAdmin, requireSuperAdmin, async (req, res) => {
  try {
    ...
    // Log audit action
    console.log(`[AUDIT] Turf ${action.toLowerCase()}: ${turfId} by ${req.admin!.email}`);

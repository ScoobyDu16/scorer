...
export class EmailService {
  ...
  private constructor() {
    ...
    // Check if we should use Resend (preferred for free tier)
  ...
  public async sendEmail(options: EmailOptions): Promise<boolean> {
    if (!this.initialized) {
      console.warn('Email service not initialized - missing API keys');
      return false;
    }
    ...
    try {
      const msg = {
        to: options.to,
        from: options.from || process.env.FROM_EMAIL || 'noreply@scorer.com',
  ...
  public async sendAdminInvitationEmail(email: string, invitationToken: string, expiryHours: number = 24): Promise<boolean> {
    if (this.useResend) {
      return this.resendService.sendAdminInvitationEmail(email, invitationToken, expiryHours);
    }
  ...
  public async sendSecurityAlertEmail(email: string, alertDetails: {
    ...
    if (this.useResend) {
      return this.resendService.sendSecurityAlertEmail(email, alertDetails);
    }
